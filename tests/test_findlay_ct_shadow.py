...
class FindlayRECMappingTests(unittest.TestCase):
    ...
    def test_pass_trigger_capture_layers_are_separate(self):
        out = mod.analyze(self.fb, self.ot)
        s = out["fox_badger_registration"]["overall"]
        self.assertAlmostEqual(s["a_R_no_trigger_given_pass"], 0.25)
        self.assertAlmostEqual(s["a_K_no_capture_given_pass"], 0.5)
        self.assertAlmostEqual(s["a_registration_failure_given_trigger"], 1 / 3)
    ...
    def test_species_composition_can_change_at_trigger(self):
        ...
        self.assertAlmostEqual(c["levels"]["FOX"]["truth_pass_proportion"], 0.5)
        self.assertAlmostEqual(c["levels"]["FOX"]["trigger_world_proportion"], 1 / 3)
