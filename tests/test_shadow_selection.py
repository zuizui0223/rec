...
class ShadowSelectionTests(unittest.TestCase):
    def test_example_estimands_are_layer_specific(self):
        ...
        # the acquisition-failure event is not double-counted as a gate failure.
        self.assertAlmostEqual(
    ...
    def test_reference_unresolved_mass_is_bounded_not_dropped(self):
        ...
        # Turn the acquired-but-gate-unevaluable w4 truth into a sampled,
        # reference-unresolved window. The analyzer must expose the resulting
        # dark mass rather than silently treating it as no-event or dropping it.
        for row in rows:
            if row["window_id"] == "w4":
