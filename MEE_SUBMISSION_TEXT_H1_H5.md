...
**1.** Ecological methods already account for imperfect detection, but automated monitoring often preserves only the event records that survive sensor triggering, registration and retention. A final event table cannot reveal the biological composition of exposure opportunities that never became rows. We developed an auditable record-entry measurement contract that links an externally defined exposure/reference world to pre-entry selection, the ecological estimand represented by retained records, downstream semantic error and correction transport.

**2.** We evaluated the contract in two observation systems with external reference information. CCTV-referenced camera-trap data provided true fox/badger and otter passes before physical trigger/capture, while protected continuous BirdVox recordings provided an enumerable acoustic exposure grid with expert event truth. We compared reference and recorded ecological estimands, standardized camera-trap composition to common physical-position distributions, constructed an oracle downstream semantic stage, and evaluated entry-aware inverse-probability correction under progressively harder held-out observation contexts with direction-swapped sham controls.

**3.** Among 881 CCTV-confirmed fox/badger passes, 51.4% failed to trigger and approximately 80.0–80.2% failed to become confirmed captures. Badger representation increased from 0.360 among reference passes to 0.439 after triggering and 0.483 after confirmed capture; the distortion persisted after standardizing reference and recorded worlds to the same camera-position distribution. Otter wet/dry selection remained after position standardization for two camera settings but disappeared for a third. In protected acoustic data, a reference late-minus-early event-window contrast of +0.131 was nearly eliminated after upstream selection even when downstream semantics were oracle-perfect. Entry-aware correction reduced composition error by 48.9% in a matched-context otter camera holdout and by 26.5% and 20.3% for fox/badger trigger and final-capture composition across held-out positions on average, but a frozen simultaneous camera-plus-position transport test worsened otter error by 19.1%.

**4.** Ecological event tables are selected measurement products rather than neutral subsets of biological events. Preserving exposure denominators, entry provenance and independent reference audits makes pre-entry selection diagnosable and can support partial correction, but both the selection function and its correction have an observation-context domain that must be tested rather than assumed. The proposed contract complements occupancy and classification-error models by retaining the pre-entry information needed to determine what ecological estimand those downstream methods actually receive.
